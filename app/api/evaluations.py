...
class BoundedCase(BaseModel):
    ...
    expected_sources: list[str] = Field(default_factory=list, alias="expectedSources", max_length=100)
...
class EvaluationWorkerUpdate(BaseModel):
    ...
    phoenix_experiment_reference: str | None = Field(default=None, alias="phoenixExperimentReference", max_length=500)
    aggregate_scores: dict[str, float] = Field(default_factory=dict, alias="aggregateScores")
    typed_failures: list[dict[str, object]] = Field(default_factory=list, alias="typedFailures", max_length=500)
...
EVALUATOR_SUITES = {
    "2026-09-10.1": {
        ...
        "deterministic": [
            "authorization_leakage", "citation_validity", "requested_language",
            "response_completeness", "refusal_typing", "source_coverage",
            "latency_budget", "dependency_classification",
        ],
        "semantic": [
            "groundedness", "answer_relevance", "completeness",
            "reference_correctness", "refusal_appropriateness",
        ],
    }
}
...
async def _authorize(project_id: str, principal, reader: GraphProjectAccessReader) -> None:
    ...
    if project_id not in access.projects:
        raise HTTPException(status.HTTP_403_FORBIDDEN, "You do not have access to this project.")


@router.post("/evaluations", response_model=EvaluationAccepted, response_model_by_alias=True, status_code=202)
async def create_evaluation(
    ...
    if request.evaluator_suite_version not in EVALUATOR_SUITES:
        raise HTTPException(status.HTTP_422_UNPROCESSABLE_ENTITY, "Unknown evaluator suite version.")
    run_id = str(uuid4())
    now = datetime.now(UTC)
    payload = {"schema_version": "1", "run_id": run_id, **request.model_dump(by_alias=False)}
    await store.create(EvaluationRunRecord(
        run_id=run_id, project_id=request.project_id, requested_by=principal.object_id,
        status="QUEUED", evaluator_suite_version=request.evaluator_suite_version,
        request_payload=payload, aggregate_scores={}, typed_failures=[],
    ...
    try:
        ...
        raise HTTPException(status.HTTP_503_SERVICE_UNAVAILABLE, "Evaluation queue is unavailable.") from failure
...
async def evaluation_status(
    run_id: str, principal: CurrentPrincipal,
    reader: Annotated[GraphProjectAccessReader, Depends(get_graph_project_access_reader)],
    store: Annotated[EvaluationRunStore, Depends(get_run_store)],
) -> EvaluationStatus:
    ...
    return EvaluationStatus(
        ...
        aggregateScores=record.aggregate_scores, typedFailures=record.typed_failures,
    )
